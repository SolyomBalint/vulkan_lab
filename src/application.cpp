...
{
    if (!fs::is_directory(cDirCache)) {
        try {
            fs::create_directory(cDirCache);
...
{
    if (theWindowManager.isVisible()) camera.update(dt);
}
...
void Application::initScene()
{
    ...
    theAssetLoader.validateAssets({ "BrainStem.glb" }, theMaterialDescriptor);
    theAssetLoader.validateAssets({ "sponza/Sponza.glb" }, theMaterialDescriptor);
    ...
    gameScene.directionalLights.push_back(DirectionalLight{ glm::normalize(glm::vec3{-0.5f, -1.0f, 0.2f}), glm::vec3(1.0f), 1.0f, "dirlight1" });
...
bool Application::initializeSwapChain()
{
    ...
    if (!ret) return false;
    theVulkanEngine.initializeSwapchainDependentComponents();
...
void Application::destroySwapChain()
{
    ...
    theVulkanEngine.destroySwapchainDependentComponents();
