...
public:
    ...
    DescriptorSetBuilder& addBinding(uint32_t binding, vk::DescriptorType type, vk::ShaderStageFlags stages, vk::Sampler* pImmutableSampler = nullptr);
    DescriptorSetBuilder& setMaximumSetCount(uint32_t count);
...
public:
    ...
    {
        while (i >= components.size()) components.push_back(std::nullopt);
        if (!components[i]) {
            components[i].emplace();
