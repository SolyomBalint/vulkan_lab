...
void GUIManager::initializeSwapChainDependentComponents()
{
    ...
    if (first) {
        ...
        info.graphicsSubmitFunction = [](const VkSubmitInfo& info, VkFence fence) {
            return (VkResult)theVulkanLayer.safeSubmitGraphicsCommand(reinterpret_cast<const vk::SubmitInfo&>(info), false, fence);
        };
        info.presentSubmitFunction = [](const VkPresentInfoKHR& info) {
            try { 
                return (VkResult)theVulkanLayer.safeSubmitPresentCommand(reinterpret_cast<const vk::PresentInfoKHR&>(info));
            } catch (...) {
