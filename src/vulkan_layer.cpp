...
{
    if (view) theVulkanLayer.device.destroy(view);
    view = nullptr;
...
VulkanImage VulkanLayer::createImage(uint32_t width, uint32_t height, uint32_t mipLevels, vk::SampleCountFlagBits numSample, vk::Format format, vk::ImageLayout layout, vk::ImageAspectFlags aspect, vk::ImageUsageFlags usage, VmaAllocationCreateInfo memoryInfo, vk::ImageTiling tiling)
{
    std::array<uint32_t, 2> queues = { queueFamilyIndices.graphicsFamily.value(), queueFamilyIndices.transferFamily.value() };
...
void VulkanLayer::createDebugMessenger()
{
    ...
    createInfo.pfnUserCallback = [] (auto messageSeverity, auto /* messageType */, auto pCallbackData, void* /* pUserData */) -> unsigned  int {
        ...
        } else if ((vk::DebugUtilsMessageSeverityFlagBitsEXT)messageSeverity == vk::DebugUtilsMessageSeverityFlagBitsEXT::eWarning) {
            vulkanLogger.LogWarning(pCallbackData->pMessage);
        } else if ((vk::DebugUtilsMessageSeverityFlagBitsEXT)messageSeverity == vk::DebugUtilsMessageSeverityFlagBitsEXT::eInfo) {
...
void VulkanLayer::createLogicalDevices()
{
    ...
    dynamicRenderingFeature.dynamicRendering = dynamicRenderingAvailable;
    dynamicRenderingFeature.pNext = &maintance4Features;

    vk::PhysicalDeviceDescriptorIndexingFeatures indexingFeatures = {};
    indexingFeatures.descriptorBindingPartiallyBound = descriptorIndexingAvailable;
    indexingFeatures.descriptorBindingVariableDescriptorCount = descriptorIndexingAvailable;
    indexingFeatures.descriptorBindingUpdateUnusedWhilePending = descriptorIndexingAvailable;
    indexingFeatures.descriptorBindingSampledImageUpdateAfterBind = descriptorIndexingAvailable;
    indexingFeatures.shaderSampledImageArrayNonUniformIndexing = descriptorIndexingAvailable;
    indexingFeatures.runtimeDescriptorArray = descriptorIndexingAvailable;
    indexingFeatures.shaderStorageBufferArrayNonUniformIndexing = descriptorIndexingAvailable;
...
void VulkanLayer::createPipelineCache()
{
    ...
    try {
        ...
        vulkanLogger.LogWarning(err.what());
    } catch (...) {}
...
bool VulkanLayer::checkDeviceExtensionSupport(const vk::PhysicalDevice& dev)
{
    ...
    for (const auto& aExt : availableExtensions) {
        ...
        } else if (strcmp(aExt.extensionName, VK_KHR_DYNAMIC_RENDERING_EXTENSION_NAME) == 0) {
            dynamicRenderingAvailable = true;
        } else if (strcmp(aExt.extensionName, VK_EXT_DESCRIPTOR_INDEXING_EXTENSION_NAME) == 0) {
            descriptorIndexingAvailable = true;
        }
    }

    if (!dynamicRenderingAvailable) {
        logger.LogWarning("Dynamic rendering is not available!");
    }    
    if (!descriptorIndexingAvailable) {
        logger.LogWarning("Descriptor indexing is not available! Textures will not be loaded!");
    }
