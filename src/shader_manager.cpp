    ...
    for (auto& p : fs::recursive_directory_iterator(cDirShaderSources)) {
        if (p.is_regular_file()) {
                ...
                ext == ".task" || ext == ".mesh" || ext == ".glsl")
            {
                ...
                else if (ext == ".task") data.stage = vk::ShaderStageFlagBits::eTaskEXT;
                else if (ext == ".mesh") data.stage = vk::ShaderStageFlagBits::eMeshEXT;
...
void ShaderManager::addAndExecuteShaderDependency(VulkanPipeline& pipeline, std::initializer_list<std::string> shaders, std::function<vk::Pipeline(vk::PipelineShaderStageCreateInfo*)> handler)
{
    auto trueHandler = [handler = std::move(handler), this, &pipeline](const std::vector<std::string>& shaders) {
        std::vector<vk::PipelineShaderStageCreateInfo> ss;
        ss.reserve(shaders.size());
        for (auto& it : shaders) {
            ss.push_back(ShaderBuilder{ find(it) }.build());
        }
...
void ShaderManager::addAndExecuteShaderDependency(VulkanPipeline& pipeline, const std::vector<std::string>& shaders, std::function<vk::Pipeline(vk::PipelineShaderStageCreateInfo*) > handler)
{
    auto trueHandler = [handler = std::move(handler), this, &pipeline](const std::vector<std::string>& shaders) {
        std::vector<vk::PipelineShaderStageCreateInfo> ss;
        ss.reserve(shaders.size());
...
void ShaderManager::addAndExecuteComputeShaderDependency(VulkanPipeline& pipeline, const std::string& shader, vk::PipelineLayout pipelineLayout, const char* name)
{
    addAndExecuteShaderDependency(pipeline, { shader }, [pipelineLayout, name](auto shaders) {
        return theVulkanLayer.name(theVulkanLayer.createComputePipeline(shaders[0].module, pipelineLayout), name);
    });
}
