...
void AssetLoader::loadGltfData(CompositeMesh& cmh, MaterialDescriptor compat, Model& model)
{
    ...
    for (int i = 0; auto& material : model.materials) {
        ...
        for (auto& desc : compat.textureDescriptors) {
            long long index = -1;
        ...
        for (auto& desc : compat.valueDescriptors) {
            double *pValue;
            switch (desc) {
...
void AssetLoader::validateGlbFile(CompositeMesh& cmh, MaterialDescriptor compat)
{
    TinyGLTF tg;
    Model model;
    std::string err, warn;
...
void AssetLoader::validateGltfFile(CompositeMesh& cmh, MaterialDescriptor compat)
{
    TinyGLTF tg;
    Model model;
    std::string err, warn;
