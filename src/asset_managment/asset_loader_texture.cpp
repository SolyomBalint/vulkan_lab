...
TextureHandler handleTextureCombination(AssetManager& am, const std::vector<std::pair<std::string, std::string>>& textures, bool mipMapped)
{
    ...
    for (size_t i = 0; i < textures.size(); ++i) {
        glm::ivec3 cdim;
        d[i] = stbi_load(textures[i].first.c_str(), &cdim.x, &cdim.y, &cdim.z, 1);
        ...
        if (!d[i]) {
            ...
            return theAssetManager.add(Texture::empty("comb_temp_name"));
        }
    }
    ...
    auto th = Texture::empty("comb_texture_temp_name");
    th.imageHandler = ih;
    th.samplerHandler = theResourceManager.defaultSampler;
