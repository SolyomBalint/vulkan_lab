...
TextureHandler AssetManager::add(Texture texture)
{
    ...
    if (theVulkanLayer.descriptorIndexingAvailable) {
...
{
    theAssetLoader.validateAssets({ name }, theMaterialDescriptor);
}
...
void AssetManager::initializeDefaultAssets()
{
    ...
    mth.textures.resize(theMaterialDescriptor.textureDescriptors.size(), dummyTexture);
    mth.values.resize(theMaterialDescriptor.valueDescriptors.size(), 0.5f);
...
void AssetManager::initializeDescriptors()
{
    ...
    layoutCreateInfo.pNext = theVulkanLayer.descriptorIndexingAvailable ? &extInfo : nullptr;
