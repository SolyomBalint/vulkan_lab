...
void AssetLoader::collectFiles()
{
    for (auto& root : rootPaths) {
        for (auto& p : fs::recursive_directory_iterator(root)) {
            if (std::string ext = p.path().extension().string(); p.is_regular_file() && !ext.empty() && loaders.contains(ext.substr(1))) {
...
void AssetLoader::validateAssets(std::vector<std::string> names, MaterialDescriptor materialDescriptor)
{
    ...
    for (size_t i = 0; i < names.size(); completedAssets = completedAssets + 1, ++i) {
        ...
        if (found == loaders.end()) {
            ...
            (this->*found->second)(mesh, materialDescriptor);
        }
    }
    scheduledAssets = scheduledAssets - names.size();
